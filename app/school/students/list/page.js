...
export default function ListStudentsPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!email && !userId) {
        alert('Email/LoginId or User ID not found. Please login again.')
        setIsLoading(false)
        return
      }
      ...
      let url = `/api/students?${queryParam}`
      if (selectedBatch) {
        url += `&batch=${encodeURIComponent(selectedBatch)}`
      }

      // Fetch data from API route with email/loginId or schoolId filter
  ...
  const handleEdit = (student) => {
    ...
    router.push(`/school/students/edit?id=${student.id}&batch=${selectedBatch}`)
  }
        ...
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
          ...
          <div className="px-8 py-6 bg-gradient-to-br from-gray-50 to-blue-50 border-b-2 border-gray-100">
            <div className="flex flex-col md:flex-row md:items-end gap-4">
              <div className="flex-1">
                <label htmlFor="batch" className="block text-sm font-bold text-gray-700 mb-2 flex items-center space-x-1">
          ...
          {students.length === 0 ? (
            <div className="p-12 min-h-[400px] flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 to-blue-50">
              ...
              <p className="text-sm text-gray-500">Click Search to view students (batch selection is optional)</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {paginatedStudents.map((student, index) => (
                    <tr key={student.id} className={`transition-all duration-150 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50 hover:shadow-sm`}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{student.cardBatch}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="text-sm text-gray-700 font-medium">
                  ...
                  {selectedRows.length > 0 && (
                    <span className="ml-4 text-blue-600 font-semibold">• {selectedRows.length} selected</span>
                  )}
                </div>
