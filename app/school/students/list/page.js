...
export default function ListStudentsPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/students?batch=${encodeURIComponent(selectedBatch)}`)
        ...
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
          ...
          {students.length === 0 ? (
            <div className="p-12 min-h-[400px] flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 to-blue-50">
              ...
              <p className="text-sm text-gray-500">Select a batch and click Search to view students</p>
            </div>
          ) : (
            ...
            <div className="px-8 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t-2 border-gray-100 text-sm text-gray-700 font-medium">
              <div className="flex items-center justify-between">
                ...
                {selectedRows.length > 0 && (
                  <span className="text-blue-600 font-semibold">{selectedRows.length} selected</span>
                )}
              </div>
            </div>
          )}
